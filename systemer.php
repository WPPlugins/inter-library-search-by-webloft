...
//****************************************************************************************************
	...
	$antallfunnet = (isset($sru_data->numberOfRecords)) ? $sru_data->numberOfRecords : null;
	return $antallfunnet;

} // end function


function mikromarc_getCountItems($xml){
	$sru_data = simplexml_load_string($xml);
	$count_itmes = (isset($sru_data->numberOfRecords)) ? $sru_data->numberOfRecords : null;
	return $count_itmes;
}
...
//*************************************************************************************************
	$result = array( 'items' => null, 'count-items' => null );
	// Vi m&aring; slenge p&aring; posisjon i URL-en
	...
	$newfile = str_replace ("</recordPosition></record>" , "</recordPosition>" , $newfile);
	...
	while ($record = $journals->next()) {
		...
		if ($record->getField("019")) {
			...
			$treff[$hitcounter]['materialkodenitten'] = substr($materialkode, 5);

			// Hvis flere adskilt med komma g&aring;r vi for den f&oslash;rste

			if (stristr($treff[$hitcounter]['materialkodenitten'], ",")) {
				$temp                               = explode(",", $treff[$hitcounter]['materialkodenitten']);
				$treff[$hitcounter]['materialkodenitten'] = $temp[0];
			}
		}

		if ($record->getField("007")) {
			$materialkode                       = $record->getField("007");
			$treff[$hitcounter]['materialkodesyv'] = trim (substr((string) $materialkode, 5));
		}
	...
	// Ikon - sjekke om materialtype finnes i 019 ELLER i 007

// http://www.bibsent.no/Files/Images/Download/BSMARC/BSMarc_utgave_4_4_2016.pdf

	// Men f&oslash;rst sette til bok, tydeligvis det som er tilfellet n&aring;r intet er oppgitt
	$treff[$hitcounter]['type'] = 'bok';

	if (isset($treff[$hitcounter]['materialkodenitten'])) { // materialkode 019 er angitt
			switch ($treff[$hitcounter]['materialkodenitten']) {
				case "ab":
				...
				case "dg":
					...
					break;
				// OBS! Ingen ting angitt = bok, virker det som
				case "":
					$treff[$hitcounter]['type'] = "bok";
					break;
				default:
		...
		if (isset($treff[$hitcounter]['materialkodesyv'])) { // materialkode 007 er angitt

			switch ($treff[$hitcounter]['materialkodesyv']) {
				case "ab":
					$treff[$hitcounter]['type'] = "atlas";
					break;
				case "vd":
		...
		if ($treff[$hitcounter]['type'] == 'dvd') { // DVD?
			$treff[$hitcounter]['tittelinfo'] .= " : DVD";
		}

		if ($treff[$hitcounter]['type'] == 'bluray') { // DVD?
	...
	*/
	...
	// _log($treff);
	//
	if ( $treff && is_array($treff) && !empty($treff) ){
		$result['items'] = $treff;
	}
...
//****************************************************************************************************
	...
	if (!method_exists($sru_data, 'getNameSpaces') ){
		return null;
	}
