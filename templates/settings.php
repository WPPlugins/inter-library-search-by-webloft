...
<div class="wrap">
  ...
  <form method="post" action="options.php">
    ...
    <tr><td colspan="2"><h3><?php _e('Visning', 'inter-library-search-by-webloft'); ?></h3></td></tr>
    <tr>
    <td><label for="wl_katalogsok_option_enkeltpost"><?php _e('Side for visning av enkeltposter:', 'inter-library-search-by-webloft'); ?></label>
    </td><td>
    ...
    if ($post_query = get_posts($args)) {
      ...
      foreach ($post_query as $post) {
        if ( stripos($post->post_content, '[wl-ils-enkeltpost') !== false ) { // kortkoden finnes!!
          $hits[$post->ID] = $post->post_title;
        }
      }

      if (is_array($hits)) { // vi har minst ett treff
        foreach ($hits as $post_id => $post_title) {
          printf ('<option value="%s" %s>%s</option>', $post_id, selected( $enkeltpost, $post_id,false ), $post_title );
        }
      }
    }
    ?>
